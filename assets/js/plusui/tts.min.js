...
classBody=localStorage.getItem("webTheme"),
...
<script src="https://cdn.jsdelivr.net/gh/ajax-jquery/sabdaliterasi.shop@master/assets/js/plusui/tts-sabda.js?o=${MYstring}"><\/script>
</body>
</html>
  `;
...
!function(e){function t(t){return e.getElementById(t)}let a=t("_google_translator_element");a.addEventListener("change",function(){var e=a.querySelector("select.goog-te-combo"),n=e.value;""==e.value&&(n="id");var l=e.options[e.selectedIndex].text;t("iframewrapper").innerHTML="<pre>Memuat Kembali Suara kedalam bahasa "+l+"...</pre>";let o=setInterval(function e(){var t=document.documentElement.lang;t==n?(submitTryit(),clearInterval(o)):"auto"==t&&(submitTryit(),clearInterval(o))},250);localStorage.setItem("Bahasa",n)});let n=localStorage.getItem("Bahasa");if(n){let l=setInterval(function e(){document.documentElement.lang==n&&(submitTryit(),clearInterval(l))},250)}else localStorage.setItem("Bahasa","id")}(document);function mtts(e){document.getElementById("iframeResult").contentWindow.document.getElementById("BodyTTS").setAttribute("class",e)};function adtts(e){var t=document.getElementById("iframeResult").contentWindow,l=t.document.getElementById("BodyTTS").getAttribute("class").replaceAll("lgT","").replaceAll("drK","").replaceAll("syD","").replaceAll(" ","");t.document.getElementById("BodyTTS").setAttribute("class",l+" "+e)}
