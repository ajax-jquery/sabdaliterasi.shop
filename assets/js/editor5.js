...
var TagSite = [{% for po in posting %}"{{ po }}"{% if forloop.last == false %},{% endif %}{% endfor %}];
autocomplete(document.getElementById("myTag"), TagSite); 

var Babi=JSON.parse(localStorage.getItem("Artikel"));
if(Babi){
 $('input[name=judul]').val(Babi.judul),
 $('input[name=tag]').val(Babi.tag),
...
localStorage.setItem("Artikel", k);  
});

$("#reset").click(function(){
$('form')[0].reset();
localStorage.removeItem("Artikel");
location.reload();window.location = window.location
});
  ...
  var image=$('input[name=image]').val().replace("http://","").replace("https://","")
...
$(p).find('br').remove();$(p).find('svg').remove();$(p).find('figure').removeAttr("contenteditable").attr("class","image_post");$(p).find('div').remove();$(p).find('img').attr("class","lazyload").removeAttr("style");
  var isi= $(p).html().split("<i>").join("<em>").split("</i>").join("</em>");
  var isidua=isi.split("<b>").join("<strong>").split("</b>").join("</strong>").split('src=').join('src="data:," data-src=').split("</figure>").join('<div class="lazy-loading"></div></figure>');
