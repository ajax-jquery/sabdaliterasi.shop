...
<div id="ms-related-post"><div class="ms-related-title"><p id="hasilo">all produk</p></div>
  ...
  {% for p in site.posts %}
 <li data-search-term="{{ p.title | replace: ' ','' | downcase }}{{ p.author | replace: ' ','' | downcase }}{{ p.description | replace: ' ','' | downcase }}" class="searproduk"><div class="msr-thumb-outer"><span class='sabda-bookmark-btn produkmark' data-quantity='1' data-borkimage='{{ p.image }}?resize=320%2C320&ssl=1' data-id='{{ p.url | replace: "/","" }}' data-link='{{ p.url | prepend: site.url }}' data-title='{{ p.title }}'><i class="fa-regular fa-bookmark"></i></span><a title="{{ p.title }}" href="{{ p.url | prepend: site.url }}"><img alt="{{ p.title }}" class="msr-thumb lazyload" data-src="{{ p.image }}" width="300" height="300" src="data:,"><div class="lazy-loading"></div></a></div><div class="msr-post-summary"><div class="msr-post-title"><a title="{{ p.title }}" href="{{ p.url | prepend: site.url }}">{{ p.title }}</a></div><div class="harga-produk">{{ p.harga }}</div><div class="Rate" data-id="{{ p.url }}"></div></div></li>
  {% endfor %}
  </ul></div>
